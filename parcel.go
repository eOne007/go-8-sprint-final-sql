...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("row with client %d not found", client)
	}
	defer rows.Close()
...
func (s ParcelStore) SetAddress(number int, address string) error {
	res, err := s.db.Exec("UPDATE parcel SET address = ? WHERE number = ? AND status = 'registered'", address, number)
	if err != nil {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("failed to update address for parcel %d: status is not 'registered'", number)
	}
	return nil
}
...
func (s ParcelStore) Delete(number int) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("parcel with number %d not found or status is not 'registered'", number)
	}
	return nil
}
